/**
 * router-bootstrap-creator: 极简触发 → 创造能力路由器。
 *
 * 首轮完全复用 router-standard standard 模式（RL 接口还原）：只有 RL 训练句 +
 * shell/str_replace_editor，contexts 清空。第一个 tool/call 后放行完整创造目录。
 *
 * 在“第一个工具调用后的第一次思考前”注入一次原版创造 persona：
 *   - 主路径：第一个 tool/result 事件（结果返回后、下一个 step claim 前 append）；
 *   - 兜底路径：第二个 tool/call 事件（第一个 tool/call 后的下一个 tool/call）。
 * 两个触发点谁先触发就注入一次；之后跳过。
 *
 * 其余逻辑（分类、weak 近场引导、dev_router_* 工具）与 router-standard 保持一致。
 *
...
export const name = 'router-bootstrap-creator'
...
/** 简单 `{{name}}` 插值：用 assembly variables 替换，未知变量保留字面量。 */
export function renderTemplate(text, variables) {
  return String(text ?? '').replace(/\{\{([^}]+)\}\}/g, (raw, name) => {
    const key = String(name).trim()
    return Object.prototype.hasOwnProperty.call(variables || {}, key) ? String(variables[key]) : raw
  })
}

/** 注入触发判定：主路径 = 第一个 tool/result；兜底 = 第二个 tool/call。 */
export function shouldInjectOnEvent(event, toolCallCount, resultCount) {
  if (event?.type === 'tool/result' && resultCount === 1) return 'primary'
  if (event?.type === 'tool/call' && toolCallCount >= 2) return 'fallback'
  return null
}
...
export function apply(ctx, config) {
  ...
  const creativePersonas = new Map() // session id -> { text, variables } captured from cordis persona
  const injectedSessions = new Set() // session id -> creative persona already injected once
  ...
  /** 会话是否已经存在本插件注入的创造 persona 消息（用于恢复去重）。 */
  function hasInjectedMessage(session) {
    return (session.events || []).some((e) => e.type === 'user/message' && e.data?.source?.plugin === 'router-bootstrap-creator')
  }

  /** 向目标 agent 的 next-step inbox 注入一次创造 persona。 */
  function injectCreativePersona(session) {
    if (injectedSessions.has(session.id)) return false
    const captured = creativePersonas.get(session.id)
    ...
    if (target === undefined || target.inbox === undefined) return false
    const text = renderTemplate(captured.text, captured.variables)
    try {
      target.inbox.append('next-step', {
        id: `router-creator-persona-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        role: 'user',
        source: { kind: 'plugin', plugin: 'router-bootstrap-creator' },
  ...
  /** 两个注入触发点：第一个 tool/result（主）或第二个 tool/call（兜底）。 */
  function maybeInjectCreativePersona(session, event) {
    if (injectedSessions.has(session.id) || hasInjectedMessage(session)) return
    const toolCalls = (session.events || []).filter((e) => e.type === 'tool/call')
    if (toolCalls.length === 0) return // 还没 promoted
    const results = (session.events || []).filter((e) => e.type === 'tool/result')
    if (shouldInjectOnEvent(event, toolCalls.length, results.length)) {
      injectCreativePersona(session)
    }
  }
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    // 捕获原版 cordis persona（含未插值变量），供稳定后注入。
    if (!creativePersonas.has(session.id)) {
      const personaSection = (assembled.sections || []).find((s) => /persona/i.test(s.name))
      if (personaSection?.text) {
        creativePersonas.set(session.id, { text: personaSection.text, variables: assembled.variables })
      }
    }
  ...
  ctx.on('session/event', (session, event) => {
    // 创造 persona 注入：主路径 = 第一个 tool/result；兜底 = 第二个 tool/call。
    if (event.type === 'tool/call' || event.type === 'tool/result') {
      maybeInjectCreativePersona(session, event)
    }
